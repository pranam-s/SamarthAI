...
| A-07 | Medium | ci.yml | `ty check` runs with `continue-on-error: true`, so type checking is advisory only; tests run without a coverage gate. | **Fixed**: type check is blocking; coverage gate added (see EVALUATION.md for threshold rationale) |
...
Gated set (core/, db/, models.py, schemas.py) remains 100%; the CI gate at ≥95% is satisfied.
