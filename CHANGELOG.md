...
- **Alembic migrations** (docs/adr/0001): `migrations/env.py` wired to
  `core.config.settings` and `models.Base.metadata` using the async engine
  recipe; baseline revision autogenerated from the models and verified
  against `create_all` output (10/10 schema objects match; downgrade to
  base and re-upgrade round-trips). Startup policy: `create_all` stays as a
  fresh-database convenience; the revision chain is the change path.
  CI gained an `alembic upgrade head` smoke step on a fresh database.
- Dead-code analysis kept active in the lint group: `deptry` (with
  package→module name mappings and justified DEP002 exemptions for
  driver/lazy-import dependencies) and `vulture` (80% confidence, `cls`
  ignored as the classmethod convention). Both run clean.
- docs/design.md (HLD + LLD), docs/BUILD_LOG.md (engineering history),
  docs/adr/0001, CONTRIBUTING.md; README rebuilt with screenshots captured
  from a real keyless run.

### Changed

- Dependency refresh via `uv lock --upgrade`: sqlalchemy 2.0.54, uvicorn
  0.53.0, google-genai 2.24.0, openai 3.15.0, pyjwt 2.14.0, pypdf 6.19.0,
  urllib3 2.8.0, ty 0.0.82, ruff 0.16.8, coverage 7.16.1, and 6 more.
- `httpx` moved from main dependencies to the dev group (only tests import
  it; the openai SDK provides it transitively at runtime).
- Accessibility: the mobile drawer toggle is a real `<button>` with
  `aria-expanded` instead of a `<label>`; chart canvases expose
  `role="img"` with localized labels (data tables remain the accessible
  source); the landing hero's external placeholder image was replaced with
  a self-contained decorative SVG (still hidden from assistive tech).

### Fixed

- The CI coverage gate was silently measuring only `core` + `db` (211
  statements): file-style `--cov=models.py`/`--cov=schemas.py` flags make
  coverage 7.16.x report "module not imported" and drop the targets.
  Module-name flags restored the documented 469-statement gate (100%).
- The test suite now runs with **zero warnings**: the wrong-key tests used
  a 26-byte HMAC key literal that tripped pyjwt's `InsecureKeyLengthWarning`
  (RFC 7518 wants ≥ 32 bytes for HS256); the alternate key is long enough
  while still testing signature rejection. CI and Docker-job test
  `SECRET_KEY` values padded for the same reason.
